...
const bucketName = "team1"; // Укажите имя вашего бакета
const key = `cat_${Math.random()}.jpg`; // Укажите ключ (путь) к файлу в S3
const fileData = fs.readFileSync('./cat.jpg'); // Файл

uploadFileToS3(bucketName, key, fileData)
    .then(() => console.log(`Файл успешно загружен в S3: https://storage.yandexcloud.net/${bucketName}/${key}`))
    .catch((error) => console.error("Ошибка при загрузке файла в S3:", error));